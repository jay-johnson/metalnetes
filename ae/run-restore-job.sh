...
if [[ "${1}" != "" ]]; then
    if [[ "${1}" == "-r" ]]; then
        ...
        if [[ ! -e ${1} ]]; then
            err "Failed to find restore job values file: ${1}"
            exit 1
        fi
        values=${1}
    fi
...
if [[ "${2}" != "" ]]; then
    if [[ "${2}" == "-r" ]]; then
        ...
        if [[ ! -e ${2} ]]; then
            err "Failed to find restore job values file: ${2}"
            exit 1
        fi
        values=${2}
    fi
...
if [[ "${recreate}" == "1" ]]; then
    anmt "deleting previous ae restore job"
    helm delete --purge ae-restore
    pod_name="restore"
...
anmt "installing ae restore job"
good "helm install --name=ae-restore ./ae-restore --namespace=${namespace} -f ${values}"
helm install \
    --name=ae-restore \
    ./ae-restore \
    --namespace=${namespace} \
